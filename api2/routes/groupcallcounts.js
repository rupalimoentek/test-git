    ...
    connectionpool.getConnection(function(err, connection) {
		...
		var sql = "select lft,rght from organizational_units where id=" +req.params.id;
			...
			if (err)
			{
				...
				var sql2 = "select indicator_scores.organizational_unit_id as 'ou_id', (select parent_id from organizational_units where id=indicator_scores.organizational_unit_id) as 'parent_id'," +
				"(select name from organizational_units where id=indicator_scores.organizational_unit_id) as 'name',"+
				"count(Distinct call_details.id) as 'call_source',"+
				"sum(case when indicator_scores.indicator_id = '73' and indicator_scores.score > '0' then 1 else 0 end) as 'sales_inquiry',"+
				"sum(case when indicator_scores.indicator_id = '18' and indicator_scores.score > '0' then 1 else 0 end) as 'conversions',"+
				"sum(case when indicator_scores.indicator_id = '50' and indicator_scores.score > '51' then 1 else 0 end) as 'lead_score',"+
				"sum(case when indicator_scores.indicator_id = '50'  then indicator_scores.score else 0 end) as 'lead_score_sum',"+
				"sum(case when indicator_scores.indicator_id = '83' and indicator_scores.score > '0' then 1 else 0 end) as 'sales_skills',"+
				"sum(case when indicator_scores.indicator_id = '83'  then indicator_scores.score else 0 end) as 'sales_skills_sum',"+
				"sum(case when indicator_scores.indicator_id = '93' and indicator_scores.score > '0' then 1 else 0 end) as 'missed_opportunity',"+
				"'0' as 'gcalldate','0' as 'gcallnum','0' as 'gsalesskills', '0' as 'gleadscore','0' as 'gconv','0' as 'gcallsum','0' as 'gaudfile','0' as 'ccalldate','0' as 'ccallnum','0' as 'csalesskills', '0' as 'cleadscore','0' as 'cmissedop','0' as 'ccallsum','0' as 'caudfile'"+
				...
				" where organizational_units.lft >=" + rows[0].lft+ " and  organizational_units.rght <=" + rows[0].rght + "  and organizational_units.status = 'active') and indicators.name != 'null'"; // AND indicator_scores.created  between '2014-09-15' and  '2014-09-16' group by indicator_scores.organizational_unit_id; 
					...
					sql2+= " group by indicator_scores.organizational_unit_id";
					console.log(sql2);
